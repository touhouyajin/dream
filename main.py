...
class GUI:
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.opt.aniso_weight > 0:
                s = self.renderer.gaussians.get_scaling      # [N, 3]
                max_s, _ = s.max(dim=1)                     # [N]
                min_s, _ = s.min(dim=1)                     # [N]
                ratio = (max_s / min_s.clamp(min=1e-6))      # [N]
                aniso_loss = ((ratio - 1.0) ** 2).mean()
                loss = loss + self.opt.aniso_weight * aniso_loss       
            # optimize step
            loss.backward()
            self.optimizer.step()
